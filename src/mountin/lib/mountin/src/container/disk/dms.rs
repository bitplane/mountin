    ...
    fn unpack_deep(&mut self, input: &[u8], output: &mut [u8]) {
        ...
        while op < output.len() {
            ...
            if c < 256 {
                ...
                let mut i = self
                    .deep_text_loc
                    .wrapping_sub(Self::deep_decode_position(&mut br))
                    .wrapping_sub(1);
                for _ in 0..j {
                    let b = self.text[(i & 0x3fff) as usize];
                    i = i.wrapping_add(1);
    ...
    fn unpack_heavy(&mut self, input: &[u8], output: &mut [u8], flags: u8) -> io::Result<()> {
        ...
        while op < output.len() {
            ...
            if c < 256 {
                ...
                let mut i = self
                    .heavy_text_loc
                    .wrapping_sub(self.heavy_decode_p(&mut br))
                    .wrapping_sub(1);
                for _ in 0..j {
