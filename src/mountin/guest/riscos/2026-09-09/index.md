...
| USBDriver → DWCDriver → SCSISoftUSB → SCSIFS | SCSI media through the Pi USB host; SCSIFS has partition-offset code | GNU fixes compile these modules into an experimental ROM, but DWCDriver does not initialize and no USB medium appears through 9d. RTSupport stalls after claiming `UnthreadV`, during its vector probe. An independent ROM module successfully claims, receives and releases the same vector; skipping RTSupport's probe lets RTSupport load. These modules are not selected by the released ROM. |
