...
| USBDriver → DWCDriver → SCSISoftUSB → SCSIFS | SCSI media through the Pi USB host; SCSIFS has partition-offset code | GNU fixes compile these modules into an experimental ROM, but DWCDriver does not finish initialization and no USB medium appears through 9d. RTSupport now loads and schedules DWCDriver's tasklet with corrected GNU declarations for its assembly entry points and shared scheduler state. DWCDriver reaches the first controller initialization delay, which has not returned in the current guest test. These modules are not selected by the released ROM. |
| CDFSDriver → CDFSSoftSCSI → CDFS | CD media on the SCSI path | Local GNU fixes compile and boot these modules in the same experimental ROM. The USB/SCSI runtime path is still blocked, so no CD media test has passed. |
