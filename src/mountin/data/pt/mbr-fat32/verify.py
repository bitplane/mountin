...
def main():
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            raise TimeoutError(
                f"TempleOS did not finish the FAT32 fixture: {observed!r}")
    finally:
    ...
    if entry[0] != 0x80 or entry[4] != 0x0B:
        raise RuntimeError("TempleOS did not publish its active FAT32 partition")
    ...
    if boot[82:90] != b"FAT32   " or boot[510:512] != b"\x55\xaa":
        raise RuntimeError("TempleOS did not format the partition as FAT32")

    print("TempleOS MBR FAT32 fixture complete")
